        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-gray-600">Tipo criado em {formatDate(type.created_at)}</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
          <div className="bg-purple-50 px-4 py-3 border-b border-purple-200 rounded-t-lg">
            <div className="flex items-center space-x-2">
              ...
              <h4 className="text-lg font-semibold text-gray-900">Informações do Tipo</h4>
            </div>
          </div>
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${type.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                {type.is_active ? (
                  <>
        ...
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-sm font-medium text-gray-900">{formatDate(type.created_at)}</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Atualizado em</label>
              <p className="text-sm font-medium text-gray-900">{formatDate(type.updated_at)}</p>
            </div>
          </div>
        </div>
      </div>
